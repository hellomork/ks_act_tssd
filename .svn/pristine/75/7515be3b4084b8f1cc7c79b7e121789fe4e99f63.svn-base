  ...
  UploadFile.prototype = {
    ...
    initUploader: function() {
      var _this = this,
        ...
        actionUrl = (upType == 'aliyun') ? editor.getActionUrl(editor.getOpt('ossFileActionName')) : editor.getActionUrl(editor.getOpt('fileActionName')),
        fileMaxSize = editor.getOpt('fileMaxSize'),
        acceptExtensions = (editor.getOpt('fileAllowFiles') || ['.txt', '.doc', '.docs', '.xls', '.xlsx', '.ppt', '.pdf', '.odt', '.ott', '.fodt', '.uot', '.xml', '.dot', '.htm', '.html', '.rtf', '.docm', '.zip', '.rar', '.tar', '.7z', '.tar.gz', '.tar.bz', '.tar.xz']).join('').replace(/\./g, ',').replace(/^[,]/, '')
      ...
      uploader.on('uploadBeforeSend', function(file, data, headers) {
        ...
        editor.getOpt('videoUploadService')(_this, editor).setFormData(file, data, headers, new_multipart_params)
      ...
      uploader.on('uploadSuccess', function(file, res) {
        ...
        if (upType == 'aliyun') {
          if (res._raw == '') {
            const fileObj = {}
            fileObj.url = oss_domain + '/' + new_multipart_params.key + '/' + timeStamp + '_' + file.name
            fileObj.name = file.name
            fileObj.title = file.name
            fileObj.type = file.ext
            fileObj.original = file.name
          ...
          try {
            if (editor.getOpt('fileUploadService')(_this, editor).getResponseSuccess(res, upType)) {
              _this.fileList.push({ url: res.data.list })
              $file.append('<span class="success"></span>')
            } else {
