  ...
  UploadVideo.prototype = {
    ...
    initUploader: function() {
      var _this = this,
        ...
        actionUrl = (upType == 'aliyun') ? editor.getActionUrl(editor.getOpt('ossVideoActionName')) : editor.getActionUrl(editor.getOpt('videoActionName')),
      ...
      uploader = _this.uploader = WebUploader.create({
        ...
        compress: false,
        fileSingleSizeLimit: editor.getOpt('videoMaxSize'), // 验证单个文件大小是否超出限制, 超出则不允许加入队列。
        threads: '1',
        chunkRetry: 2, // [可选] [默认值：2]如果某个分片由于网络问题出错，允许自动重传多少次？
        chunked: upType != 'aliyun', // 是否分片
        chunkSize: (upType == 'aliyun') ? 0 : (20 * 1024 * 1024) //
      })
      uploader.addButton({
        id: '#filePickerBlock'
      })
      ...
      uploader.on('uploadSuccess', function(file, res) {
        ...
        if (upType == 'aliyun') {
          if (res._raw == '') {
            const videoObj = {}
            videoObj.url = oss_domain + '/' + new_multipart_params.key + '/' + timeStamp + '_' + file.name
            uploadVideoList.push(videoObj)
            $file.append('<span class="success"></span>')
          }
        } else {
          try {
            if (editor.getOpt('videoUploadService')(_this, editor).getResponseSuccess(res, upType)) {
              uploadVideoList.push({ url: res.data.list })
              $file.append('<span class="success"></span>')
            } else {
