...
export default {
  ...
  methods: {
    ...
    afterRead(file, detail) {
      const that = this
      // if(this.isMobile){
      //   that.uid = file.lastModified
      // }else{
      //   that.uid = file.file.uid
      // }
      // var imglist = []
      // debugger
      this.getSignature(() => {
        ...
        // this.ossParams.dir + '/' +
        var filename = Date.parse(new Date()) + '_' + this.random_string(6) + getSuffix(file.file.name)
        ...
        formData.append('expire', this.ossParams.expire)
        ...
        $.ajax({
          ...
          success: function(data) {
            console.log('上传返回值', data)
            file.percentage = 100
            file.status = 'success'
            file.message = '上传成功'
            that.$emit('uploadSuccess', data.data.list[0], file.file.uid, that.subscript)
          },
          error: function(data) {
            console.log(data)
            file.status = 'failed'
    ...
    exceed(files, fileList) {
      this.$message.error('超出限制，只能选1张图片，请删除后再添加')
    },
    // 覆盖默认的上传行为，自定义上传的实现
    uploadPc(file) {
      this.afterRead(file)
