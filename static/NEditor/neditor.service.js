...
UE.Editor.prototype.getActionUrl = function(action) {
  ...
  if (action == 'uploadimage' || action == 'uploadscrawl' || action == 'uploadvideo' || action == 'uploadfile') {
    return window.location.origin + '/common/upload.login' // UE.Editor.prototype.getOpt('localUrl')
  } else if (action == 'ossimage' || action == 'ossvideo' || action == 'ossfile') {
    return 'https://jnszwhg.oss-cn-beijing.aliyuncs.com'// UE.Editor.prototype.getOpt('ossUrl')
  } else {
    // return 'http://www.jlswhg.com/ueditor/ueditorServer.show';
...
window.UEDITOR_CONFIG['imageUploadService'] = function(context, editor) {
  return {
    ...
    /**
         ...
         * @returns 上传参数对象
         */
    setFormData: function(object, data, headers, new_multipart_params) {
      data = Object.assign(data, new_multipart_params)
      return data
    },
    /**
         * 触发startUpload事件时执行
    ...
    /**
         ...
         */
    getResponseSuccess: function(res, upType) {
      if (upType == 'local') {
