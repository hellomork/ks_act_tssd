...
export default {
  ...
  methods: {
        ...
        if (file.length > 1) {
          file.forEach(item => {
            this.getSignature(() => {
              ...
              var filename = this.ossParams.dir + '/' + Date.parse(new Date()) + '_' + this.random_string(6) + getSuffix(item.file.name)
              console.log('item', item, item.file.name)
              ...
              $.ajax({
                ...
                success: function(data) {
                  ...
                  item.status = 'success'
                  item.message = '上传成功'
                  that.$emit('uploadSuccess', fileUrl, item.file, that.subscript)
                },
                error: function(data) {
                  console.log(data)
                  item.status = 'failed'
    ...
    exceed(files, fileList) {
      this.$message.error('超出限制，只能最多选八张图片，请删除后再添加')
    },
    // 覆盖默认的上传行为，自定义上传的实现
    uploadPc(file) {
      this.afterRead(file)
